...
struct VehicleParams
{
    ...
    VehicleParams()
        : mu(1.0489), csf(4.718), csr(5.456), lf(0.15875), lr(0.17145), h(0.074), mass(3.74), iz(0.04712), g(9.81), alphaSat(0.19), fxSplit(0.45), minVelocity(-5.0), maxVelocity(7.5), minAcceleration(-10.0), maxAcceleration(9.51), minSteeringAngle(-0.5236), maxSteeringAngle(0.5236) // ~52.7 degrees
    {
    }
};
...
inline VehicleState stepDynamicsCPU(VehicleState state, const VehicleParams &params, const ControlInput &control, float dt)
{
    VehicleState nextState = state;

    double vx_safe = (std::fabs(state.vx) < 0.1) ? 0.1 : state.vx;
